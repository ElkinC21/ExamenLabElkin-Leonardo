...
public class Adminpantalla extends JFrame {
    ...
    private void initUI() {
        ...
        right.add(new JLabel("Título:")); right.add(tfTitle);
        right.add(new JLabel("Género:")); right.add(tfGenre);
        right.add(new JLabel("SO (W/M/L):")); right.add(tfSO);
        right.add(new JLabel("Edad mínima:")); right.add(tfAge);
        ...
        btnReporte = new JButton("Generar reporte cliente (.txt)");

        btnSetEstado.addActionListener(this::onSetEstado);
        btnReporte.addActionListener(this::onReporte);

        players.add(new JLabel("Código de jugador:")); players.add(tfPlayerCode);
        players.add(cbActivo);
        players.add(btnSetEstado);
        players.add(new JLabel("Reporte: se guardará donde indiques"));
        players.add(btnReporte);

        tabs.addTab("Gestión de juegos", games);
    ...
    private void refreshGames() {
        gamesModel.clear();
        try {
            List<Juego> all = steam.printGames();
    ...
    private void onAddGame(ActionEvent e) {
        try {
            ...
            Juego g = steam.addGame(t, ge, so, edad, price, foto);
    ...
    private void onUpdPrice(ActionEvent e) {
        ...
        try {
            ...
            if (steam.updatePriceFor(sel.code, price)) {
    ...
    private void onSetEstado(ActionEvent e) {
        try {
            ...
            boolean ok = steam.setPlayerActive(code, cbActivo.isSelected());
            JOptionPane.showMessageDialog(this, ok ? "Estado actualizado." : "No se encontró jugador.");
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void onReporte(ActionEvent e) {
        try {
            ...
            if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                boolean ok = steam.reportForClient(code, fc.getSelectedFile().getAbsolutePath());
